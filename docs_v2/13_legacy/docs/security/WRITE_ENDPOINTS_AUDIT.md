...
| `/pn-review/:sku/apply-decision` | POST | todos los `engine_*.json` con el PN | `{action}` o `{estado,accion}` | action/estado/accion canÃ³nicos | decision incorrecta propagada a todo el PN | alta | Si | Si |
...
| `/pn-review/by-id/:id/apply-decision` | POST | un `engine_*.json` | `{action}` o `{estado,accion}` | action/estado/accion canÃ³nicos + lookup por ID | write puntual a fila incorrecta | alta | Si | Si |
...
| `/qa_revision_sync.php` | POST | `qa_revision_server_data.json` | objeto revision sync | objeto no vacio + tamaÃ±o acotado | persistencia auxiliar corrupta | media | Si | No |
| `/audit-log` | POST | `qa_audit_log.json` | objeto auditoria | objeto no vacio + tamaÃ±o acotado | log corrupto o demasiado grande | media | Si | No |
