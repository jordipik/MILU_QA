...
# Contrato de endpoints crÃ­ticos

> **CONTRATO MILU â€” v1** Â· Fase: CONTRATOS + ESTABILIDAD Â· No modifica cÃ³digo ni datos.
>
> Tabla de referencia oficial de los endpoints HTTP del backend Express ([server.js](../../server.js)). Incluye entrada, salida, efectos en disco, archivos afectados, nivel de riesgo y validaciÃ³n recomendada.

## Leyenda

- **Disco**: Â¿el endpoint escribe en disco? `R` = solo lectura Â· `RW` = lectura+escritura Â· `â€“` = sin acceso a disco.
- **Riesgo**: `bajo` (idempotente, sin efectos), `medio` (modifica disco con scope acotado), `alto` (modifica engine_*.json o lanza procesos).
- Todos los endpoints responden JSON salvo indicaciÃ³n contraria.

## 1. Sistema y catÃ¡logo

| Endpoint | MÃ©todo | Entrada | Salida | Disco | Archivos | Riesgo | ValidaciÃ³n recomendada |
|---|---|---|---|---|---|---|---|
| `/health` | GET | â€” | `{ok, service}` | â€“ | â€” | bajo | smoke test |
| `/version` | GET | â€” | `{version}` | R | [package.json](../../package.json) | bajo | comparar con package.json |
| `/engines` | GET | â€” | `{ok, engines[], totals{rowCount,fileSize}}` | R | 9Ã— `engine_*.json` (stat) | bajo | comprobar 9 entries |
| `/api/esquemas-pos-index` | GET | â€” | `{ok, files[]}` | R | [esquemas_pos_circulos/](../../esquemas_pos_circulos/) (cacheado) | bajo | comprobar lista no vacÃ­a |

## 2. RevisiÃ³n QA

| Endpoint | MÃ©todo | Entrada | Salida | Disco | Archivos | Riesgo | ValidaciÃ³n recomendada |
|---|---|---|---|---|---|---|---|
| `/qa_revision_sync.php` | GET | â€” | `{meta, revisions{v,r[],k{}}}` | R | [qa_revision_server_data.json](../../qa_revision_server_data.json) | bajo | JSON vÃ¡lido + meta.updated_at |
| `/qa_revision_sync.php` | POST | `{revisions{v,r[],k{}}}` | `{ok, saved_rows}` | RW | [qa_revision_server_data.json](../../qa_revision_server_data.json) | medio | round-trip GETâ†’POSTâ†’GET |
| `/save-json` | POST | `{file, id, field, value}` | `{ok}` | RW | un `engine_*.json` | alto | verificar fila despuÃ©s + valores `qa_revision_*` âˆˆ contrato |
| `/save-json.php` | POST | (alias) | igual | RW | igual | alto | igual |
| `/apply-revision-to-engines` | POST | `{rows[], k{}}` | `{ok, result}` | RW | mÃºltiples `engine_*.json` | alto | dry-run previo + diff de filas |
...
| Endpoint | MÃ©todo | Entrada | Salida | Disco | Archivos | Riesgo | ValidaciÃ³n recomendada |
|---|---|---|---|---|---|---|---|
| `/pn-review/list` | GET | query `?decision=&limit=` | `{ok, rows[], total, loaded_at}` | R (cache) | 9Ã— `engine_*.json` (cargados en memoria) | bajo | comprobar total > 0 |
...
| `/pn-review/:sku/sources` | GET | path `:sku` | `{ok, sku, count, rows[]}` | R | cache | bajo | count == nÂº filas reales |
| `/pn-review/:sku/apply-decision` | POST | `{action: validar\|revisar\|descartar}` | `{ok, sku, target_estado, target_accion, decision_applied, rows_updated}` | RW | engines que contienen al SKU | alto | post-condiciÃ³n: `estado/accion` canÃ³nicos ([CONTRATO_REVISION_QA.md](CONTRATO_REVISION_QA.md)) |
...
| `/pn-review/apply-siblings-bulk` | POST | `{}` | `{ok, result{scanned_items, pns_with_changes, planned_updates,...}}` | RW | mÃºltiples `engine_*.json` | alto | dry-run + log de cambios |
...
| `/audit-log` | POST | `{action, ...}` | `{ok}` | RW | igual | medio | mÃ¡ximo 10000 entradas (rotaciÃ³n) |
| `/audit-log` | DELETE | â€” | `{ok}` | RW | igual | alto | confirmar antes de purgar |
...
Respuesta estÃ¡ndar: `{ok: false, legacy: true, error: "...desactivado..."}`.

## 7. Reglas de oro

1. **El servidor debe atender `/qa_revision_sync.php` ANTES del static middleware** (si se sirve PHP estÃ¡tico por error, devuelve el archivo fuente en vez de JSON).
...
3. **Endpoints de alto riesgo** deben tener:
   - validaciÃ³n de payload de entrada,
...
## 8. Orden de diagnÃ³stico oficial

Ante un fallo:

1. `GET /health` â†’ servidor vivo.
2. `GET /version` â†’ versiÃ³n correcta.
3. `GET /qa_revision_sync.php` â†’ JSON, NO el archivo PHP fuente.
4. `POST /save-json` o `POST /apply-revision-to-engines` â†’ segÃºn el flujo afectado.
5. UI ([qa_milu.html](../../qa_milu.html)) en Ãºltimo lugar.

Para persistencia: servidor levantado â†’ respuesta HTTP â†’ payload del frontend â†’ escritura efectiva en disco.

## 9. Riesgos / pendientes

- **R1**: No hay validaciÃ³n formal de esquemas para los payloads POST. Recomendado: aÃ±adir `ajv` o equivalente en una fase posterior.
