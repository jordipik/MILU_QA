...
const { getExportField, getExportType, isExportable } = require('../lib/export-field-helper');
const { runUpdateFgFgs } = require('./update_fg_fgs_fields');

const REPO_ROOT = path.resolve(__dirname, '../../../..');

const DATA_JSON_DIR = path.join(REPO_ROOT, 'src', 'data', 'json');
const OUTPUT_DIR = path.join(REPO_ROOT, 'src', 'data', 'legacy-data', '05-wordpress');
const AUDIT_OUTPUT_DIR = path.join(REPO_ROOT, 'src', 'data', 'legacy-data', 'data', 'output', 'wordpress');
const FG_FGS_CATALOG_PATH = path.join(DATA_JSON_DIR, 'EXCEL_FG-FGS.json');
...
function loadEngineRows() {
    ...
    for (const fileName of ENGINE_JSON_FILES) {
        ...
        if (!Array.isArray(parsed)) {
            console.warn('[EXPORT WP] Engine no cargado o inválido:', filePath);
            continue;
        }
...
function buildQaSummary(rows) {
    ...
    for (const row of rows) {
        ...
        if (estado === 'pendiente' || estado === 'en revision' || estado === 'en revisiÃ³n') {
...
function isQaPendingOrReviewRow(row) {
    ...
    if (estado === 'pendiente' || estado === 'en revision' || estado === 'en revisiÃ³n') return true;
...
function writeOutputs(dirPath, payload) {
    ...
    const safeDiscardedRows = Array.isArray(discardedRows) ? discardedRows : [];
    ...
    writeJson(path.join(dirPath, 'milu_wp_pending_review.json'), safePendingRows);
    writeJson(path.join(dirPath, 'milu_wp_discarded.json'), safeDiscardedRows);
    ...
    writeCsv(path.join(dirPath, 'milu_wp_pending_review.csv'), safePendingRows, headers);
    writeCsv(path.join(dirPath, 'milu_wp_discarded.csv'), safeDiscardedRows, headers);
    ...
    writeJson(path.join(dirPath, 'milu_wp_pending.json'), safePendingRows);
    ...
    writeCsv(path.join(dirPath, 'milu_wp_pending.csv'), safePendingRows, headers);
