...
function applyToRow(row, options) {
    ...
    if (options.updateRevision) {
        ...
        if (isNoiseFooter) {
            // Registros footer/ruido: siempre marcar OK/Eliminar (salvo que ya estÃ© ok/eliminar y no se fuerza)
            ...
            // Si ya fue revisado manualmente como 'ok', respetar esa decisiÃ³n (salvo forceRevision)
            const currentEstado = String(row?.qa_revision_estado || '').trim().toLowerCase();
