...
export function getAppRootUrl() {
    ...
    return new URL(`${getAppBasePath() || ''}/`, origin).href;
}
...
export function getResourceUrl(resourceName) {
    ...
    const mapped = mapRootResourcePath(normalized);
    return new URL(mapped, getAppRootUrl()).href;
}

function mapRootResourcePath(resourceName) {
    if (/^engine_[^/]+\.json(?:\.bak\.[^/]*)?$/i.test(resourceName)) {
        return `src/data/json/${resourceName}`;
    }
    if (/^(MILU_|product-export-|qa_synthetic_|qa_revision_|qa_audit_log|df_).*\.json$/i.test(resourceName)) {
        return `src/data/json/${resourceName}`;
    }
    if (resourceName.startsWith('pdf/')) {
        return `src/assets/${resourceName}`;
    }
    if (
        resourceName.startsWith('fotos_articulos/')
        || resourceName.startsWith('fotos_motores/')
        || resourceName.startsWith('esquemas/')
        || resourceName.startsWith('esquemas_pos_circulos/')
    ) {
        return `src/assets/${resourceName}`;
    }
    if (resourceName.startsWith('tools/')) {
        return `src/tools/${resourceName.slice('tools/'.length)}`;
    }
    if (resourceName === 'version.json') {
        return 'src/data/json/version.json';
    }
    return resourceName;
}
...
function getSaveBackendCandidateUrls() {
    ...
    const phpCandidate = getResourceUrl('save-json.php');
...
function getDeleteBackendCandidateUrls() {
    ...
    const phpCandidate = getResourceUrl('delete-json.php');
