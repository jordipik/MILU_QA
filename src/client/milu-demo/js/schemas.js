...
import { getResourceUrl } from './data-loader.js';
...
export function buildSchemaImageCandidates(bookValue, schemaToken) {
    ...
    if (/\.(png|webp|jpg|jpeg)$/i.test(token)) {
        const direct = getResourceUrl(`${folder}${encodeURIComponent(token)}`);
        seen.add(direct); paths.push(direct);
    }
    names.forEach(name => {
        extensions.forEach(ext => {
            const path = getResourceUrl(`${folder}${encodeURIComponent(name)}.${ext}`);
...
export function buildSchemaPosImageCandidates(bookValue, rawTokenOrPath) {
    ...
    names.forEach(name => { extensions.forEach(ext => { pushPath(getResourceUrl(`${folder}${encodeURIComponent(name)}.${ext}`)); }); });
    if (tokenExt) {
        names.forEach(name => { pushPath(getResourceUrl(`${folder}${encodeURIComponent(name)}.${tokenExt}`)); });
    }
    return paths;
}
...
export function getPosSchemasForRow(row) {
    ...
    const getExistingCandidates = (candidates) => {
        ...
        if (!state?.esquemasPosFileSet) return [];
        if (state.esquemasPosFileSet.size === 0) {
            return state.esquemasPosIndexLoaded ? [] : candidates;
        }
...
function updateSchemaMarkerLink(row) {
    ...
    if (!row) {
        link.hidden = true;
        link.href = getResourceUrl('tools/simple_scheme_circle_marker.html');
        return;
    }
    ...
    link.href = `${baseHref}${params.toString() ? `?${params.toString()}` : ''}`;
    link.hidden = false;
}
