...
5. Si se tocan confirmaciones UX-4: validar manualmente cancelar, Escape, texto incorrecto, texto correcto y ausencia de doble ejecuciÃ³n.
6. Si se tocan toasts UX-3: validar `success/error/warning/info`, cierre manual, autocierre y que UX-4 siga exigiendo texto tipado en acciones crÃ­ticas.
7. Si se cierra UX-3 fase 2: verificar por bÃºsqueda que no quedan `alert(` directos en `js/bulk-revision-helper.js`, `js/cell-editor.js` y `js/revision.js`.
...
- `npm run test:field-registry`
  - CuÃ¡ndo usarlo: al tocar `fieldAdapter`, la normalizacion de campos o lecturas compatibles nuevo/legacy en UI.
  - QuÃ© valida: tests de migracion JSON y compatibilidad de lectura de campos.

- `npm run compare:normalized`
  - CuÃ¡ndo usarlo: despues de cambios en reglas de normalizacion, comparador o aliases que afecten equivalencia funcional.
  - QuÃ© valida: consistencia funcional entre `engine_*.json` y `data/normalized/*.normalized.json` con resumen OK/CHECK.

- `npm run refactor:json:dry`
  - CuÃ¡ndo usarlo: antes de ejecutar una normalizacion real, para revisar impacto sin escribir archivos de salida.
  - QuÃ© valida: que el pipeline de refactor puede procesar todos los engines sin modificar datos en disco.
