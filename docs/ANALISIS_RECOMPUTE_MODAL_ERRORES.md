...
- `server.js::POST /recompute-qa-errors`
  - Valida payload y despacha a recálculo por libro o global.
...
### Recálculo local en memoria, no usado por la opcion 3 para persistir
...
## 5. Leyes del recálculo local en memoria `error-recalc.js`
...
- Si el recálculo persiste cambios en `engine_*.json`, el backend hace backup por archivo usando sufijo `.backup`.
- El recálculo local en memoria (`error-recalc.js`) no escribe nada en disco.
