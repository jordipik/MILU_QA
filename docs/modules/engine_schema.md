...
```
✓ engine_12V4000M40A.json     2759 registros  OK
✓ engine_12V4000M53.json      6580 registros  OK
✓ engine_12V4000M70.json      5358 registros  OK
✓ engine_16V4000M61.json      4987 registros  OK
✓ engine_16V4000M73.json     12445 registros  OK
✓ engine_16V4000M73L.json    11128 registros  OK
✓ engine_16V4000M90.json      2851 registros  OK
✓ engine_20V4000M93.json     14643 registros  OK
✓ engine_20V4000M93L.json     7132 registros  OK
...
|---|---|---|
| `ID` | `string\|integer` | Identificador único. Puede ser numérico o string numérico (legacy) |
...
| `qa_revision_estado` | `string` (enum) | Estado de revisión QA |
| `qa_revision_accion` | `string` (enum) | Acción de revisión QA |
...
| Campo | Tipo | Notas |
|---|---|---|
| `POS` | `string\|null` | Posición original |
| `PART NO.` | `string\|null` | Part number original |
| `DESIGNATION` | `string\|null` | Denominación original |
| `QTY` | `string\|null` | Cantidad |
| `UNITS` | `string\|null` | Unidades |
| `WEIGHT` | `string\|null` | Peso con unidades (ej. "15 G") |
| `FN` | `string\|null` | Código función |
| `MEASUREMENT / STANDARD` | `string\|null` | Medida/norma BOM |
| `FG/FGS` | `string\|null` | Código FG/FGS |
| `BOM-No.` | `string\|null` | Número BOM |
| `Source Page` | `number\|string\|null` | Página fuente (legacy: string) |
...
| Campo | Tipo | Notas |
|---|---|---|
| `pn_raw` | `string\|null` | PN antes de normalización |
| `pn_final` | `string\|null` | **Editable**. PN definitivo |
| `criterio_pn` | `string` (enum)\|null | Criterio de normalización |
| `pos_final` | `number\|string\|null` | Posición normalizada |
| `designation_final` | `string\|null` | **Editable**. Denominación final |
...
| `depuracion_ts` | `string (date-time)\|null` | Timestamp depuración |

> **Regla `measure_final`**: prioridad `dimensions_gesa` si existe; si no, usa `MEASUREMENT / STANDARD`.  
> Normalización: colapsar espacios múltiples (ej. `A  55   X  5` → `A 55 X 5`).

#### Campos de error (calculados por `add_final_fields.py`)

| Campo | Tipo | Notas |
|---|---|---|
| `pos_error` | `integer >= 0` | Contador errores posición |
| `pn_error` | `integer >= 0` | Contador errores PN |
| `designation_error` | `integer >= 0` | Contador errores denominación |
...
|---|---|---|---|
| `qa_revision_estado` | `string` | ✅ | `"ok"`, `"pendiente"` |
| `qa_revision_accion` | `string` | ✅ | `"importar"`, `"revisar"`, `"copia"`, `"eliminar"` |
| `qa_revision_updated_at` | `string (date-time)` | ✅ | ISO 8601 |
| `qa_revision_confianza` | `number 0-1\|null` | — | Score auto-revisión |
| `qa_revision_motivo` | `string\|null` | — | Motivo auto-revisión |
| `qa_revision_origen` | `string\|null` | — | `"auto"` u otro |
| `qa_revision_regla_aplicada` | `string\|null` | — | Código de regla |
| `qa_revision_fecha` | `string (date-time)\|null` | — | Timestamp revisión auto |

#### Campos catálogo GESA

| Campo | Tipo | Notas |
|---|---|---|
| `gesa` | `"SI"\|"NO"\|null` | ¿Existe en GESA? |
| `designation_gesa` | `string\|null` | Denominación GESA |
| `nsn` | `string\|null` | NATO Stock Number |
| `normalizado` | `"SI"\|"NO"\|null` | ¿Artículo normalizado? |
| `dimensions_gesa` | `string\|null` | Dimensiones GESA (prioridad para `measure_final`) |
| `weight_gesa` | `number\|string\|null` | Peso GESA (puede ser numérico o string legacy) |
| `units` | `string\|null` | Unidades (ej. "KGM") |
| `existeix_gesa` | `boolean\|null` | Flag presencia en GESA |

#### Campos catálogo Sustituciones (SUST/MTU)

| Campo | Tipo | Notas |
|---|---|---|
| `existeix_sust_new` | `boolean\|null` | ¿Existe como New PN? |
| `existeix_sust_old` | `boolean\|null` | ¿Existe como Superseded PN? |
| `New Part Number` | `string\|null` | Nuevo PN MTU |
| `Superseded Part Number` | `string\|null` | PN supersedido MTU |
| `Hierarchie` | `"New"\|"Superseded"\|null` | — |
| `Replacement Type` | `"F"\|"I"\|null` | Tipo reemplazo MTU |
| `sust_status` | `string\|null` | `"SI"` o `""` |
| `sust_hierarchie` | `"New"\|"Superseded"\|null` | — |
...
> **Nota legacy**: `normalizado_pdf` y `gesa_pdf` pueden tener valor `"No"` (minúscula) en datos legacy.
...
| `exp_imagenes` | `string\|null` | **Editable**. JSON serializado de exportación |
| `esquemas` | `string\|null` | Esquemas eléctricos (serializado) |
| `esquemas_circulos` | `string\|null` | Círculos activos (serializado) |
| `esquemas_circulos_all` | `string\|null` | Todos los círculos (serializado) |
| `ruta_esquemas_pos` | `string\|null` | Rutas por posición (serializado) |

#### Campos calculados adicionales

| Campo | Tipo | Notas |
|---|---|---|
| `status` | `string` (enum)\|null | Estado catálogo: `EMPTY`, `NOISE`, `OK_GESA`, `OK_SUST_NEW`, `OK_SUST_OLD`, `REVISAR` |
| `fg_code` | `number\|null` | Código numérico FG |
| `fgs_description` | `string\|null` | Descripción FGS |
...
| Alias | Campo canónico | Gestión |
|---|---|---|
| `measurement_final` | `measure_final` | `FIELD_ALIASES` en `server/validation/allowed-fields.js` |

---

## Campos editables vía `/save-json`

Solo estos campos pueden ser modificados por el endpoint de edición puntual:
...
1. **`ID` puede ser string o integer**: legacy contiene IDs numéricos como strings. El validador acepta ambos.
2. **`weight_gesa` puede ser number o string**: datos GESA incluyen pesos numéricos (kg) y strings legacy.
...
5. **`qa_revision_updated_at` no es required**: registros pre-revisión no tienen este campo.
6. **`additionalProperties: true`**: el esquema no rechaza campos extra para no bloquear evolución futura.
...
- **CI estricto (DT-4+)**: cuando todos los engines sean conformes, activar validación en `npm run check`.  
- **Python tooling**: `depuracion_json.py` y `add_final_fields.py` pueden incorporar validación opcional usando el mismo esquema (vía `jsonschema` o validación manual).
