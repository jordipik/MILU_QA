...
## Objetivo
Aislar completamente la lógica experimental reciente de detección de columnas en `analista_02.html` para volver a una base mínima y estable del visor PDF.

## Lógica desactivada o retirada
- Se desactiva por flag interno en `js/pdf-viewer.js` toda la ruta experimental de:
  - detección de columnas por header anchors
...
- Las APIs públicas se mantienen por compatibilidad, pero quedan en modo no operativo:
...
- En `js/analista-02.js` se elimina el cableado de UI que reactivaba la lógica experimental.
- En `analista_02.html` se retiran los controles visibles de:
  ...
  - panel de logs de detección de cabecera
...
- Búsqueda de `pn_final` para el registro activo.
- Marcado azul de la línea donde aparece `pn_final`.
- Conservación de los resaltados estables ya existentes del visor, incluido el resaltado verde cuando la coincidencia exacta ya estaba soportada por el flujo normal.
- Caja de "Textos azules" con acciones de cargar/copiar, usando solo los highlights de línea activos.
- Comparativa Excel/GESA/SUBST/PDF/Final sin cambios funcionales fuera del reset del detector experimental.

## Qué se deja preparado para una nueva implementación desde cero
- Las APIs viejas no se borran del todo, para evitar roturas por imports o llamadas residuales.
- El punto estable para un nuevo detector queda separado del flujo mínimo actual:
  - selección PDF estable
  - extracción de `textItems`
  - render de highlights estables
  - overlay experimental actualmente anulado
- El nuevo trabajo puede reintroducir un detector nuevo sin depender del código visual experimental anterior.
...
- Comprobación manual en navegador sobre `http://localhost:3000/analista_02.html`
  - en una carga fresca ya no aparecen los controles de columnas/debug retirados
  - al activar `Marcar línea PN en PDF` se observan highlights de línea (`lineHighlights: 8`, `pnHighlights: 1`, `redRows: 1`)
...
- Si alguna pestaña vieja del navegador mantiene recursos cacheados, puede seguir mostrando controles antiguos hasta recargar.
- `pdf-table-parser.js` queda sin uso activo en este flujo; conviene tratarlo como legado temporal hasta la nueva implementación.
