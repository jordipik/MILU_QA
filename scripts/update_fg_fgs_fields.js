...
function resolveFgFgsPath(repoRoot, suppliedPath = '') {
    ...
    if (input) {
        ...
        candidates.push(path.join(repoRoot, 'EXCEL_FG-FGS.json'));
        candidates.push(path.join(repoRoot, 'data', 'EXCEL_FG-FGS.json'));
    }
    ...
    throw new Error(`No se encontro EXCEL_FG-FGS.json. Rutas probadas: ${candidates.join(', ')}`);
}
