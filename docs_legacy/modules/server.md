...
- Provide PN Review API (`/pn-review/*`):
  - `GET /pn-review/list`: lista de PNs con conteo, motores, y estado de decisión consolidado.
  ...
  - `POST /pn-review/:sku/apply-decision { action }`: aplica `validar`→ok/importar, `revisar`→pendiente/revisar, `descartar`→ok/eliminar en TODAS las filas del PN en todos los engine JSON. Devuelve `{ok, sku, decision_applied, rows_updated, files_touched, errors}`.

## Special Cases / Risks
- `/save-json` depends on row ID and exact file mapping.
- `/pn-review/:sku/apply-decision` es una operación destructiva multi-archivo; valida whitelist de acciones (400 si inválida).
