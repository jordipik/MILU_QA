...
## Qué falta antes de pruebas reales ampliadas
...
## Qué falta antes de producción estable
...
|---|---|---|---|
| Regresiones por alto acoplamiento en monolitos | Alta | Alta | Cambios pequeños impactan áreas no previstas |
...
| Confusion operativa por docs/rutas legacy | Media | Alta | onboarding y soporte más lentos |
...
1. Resolver delta JSON vs SQLite mirror y añadir gate automatico db:validate.
2. Reducir alert() legacy en módulos QA de mayor uso.
3. Alinear docs/rutas reales (retirar referencias a pn_review.html, qa_web, milu_qa).
4. Añadir validacion semantica mínima para payload masivo de revisiones.
...
4. Plan de reducción de alert() y normalizacion de notificaciones QA.
