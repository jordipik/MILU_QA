...
- Geometría de tokens y líneas
- Área tabular detectada
- Headers identificados
- Boundaries de columnas
- Asignación de tokens a columnas
...
PDF Canvas (rendered page)
    ↓
┌──────────────────────────────┐
│ SVG Overlay Layer            │  ← geométrico (rects, lines, text labels)
│ (above canvas)               │
└──────────────────────────────┘
    ↓
┌──────────────────────────────┐
│ Selection Layer (divs)       │  ← interactive highlights
│ .pdf-selection-layer         │  ← token highlighting on hover
└──────────────────────────────┘
```
...
Cada columna tiene color único para identificación rápida:
...
```javascript
// Línea vertical para cada boundary
columns.forEach((col) => {
    // Left edge
    drawLine(svg, {
        x1: col.x1,
...
```javascript
// Líneas mostrando detected separators
...
```javascript
// Después de column assignment, pintar cada token con color de columna
rows.forEach((row) => {
...
<div id="headerDetectionPanel" class="header-detection-panel">
    ...
    <table>
        ...
        <!-- Rows dinámico desde state.currentPdfHeaderOnlyDebug -->
    </table>
</div>
```
...
<button id="recalculateTableBtn" title="Recalcular detección completa">
    Recalcular Tabla
</button>

<button id="showTableStatsBtn" title="Mostrar estadísticas de detección">
    Estadísticas
</button>
...
- **Canvas**: Mejor para muchos rects pequeños (bitmap, rápido)
- **SVG**: Mejor para líneas, labels, interactivity (vectorial)

**Current approach**: SVG overlay encima de canvas para manejo híbrido.
...
**Véase también:**
- [08_runtime_flow.md](08_runtime_flow.md) - Cómo se invocan las visualizaciones
