...
Después de construir el grid de filas/columnas, aplicar validaciones semánticas y overrides para corregir errores de asignación geométrica mediante lógica de negocio.

**Examples of semantic override**:
- Token "AB" en columna `weight` → reclasificar a `fn` (porque AB es válido FN code)
- Token "EM" en columna `weight` → reclasificar a `fn` (porque EM es válido FN code)
- Token all-digits en `designation` → validar contra part number patterns

## FN Whitelist Override

**Problem**: FN (Footnote) es una columna estrecha que contiene codes cortos (2-3 chars, típicamente letras + números). Si está mal detectada geométricamente, codes pueden asignarse a columna adyacente (ej: `weight`).

**Solution**: Lista canónica de válidos FN codes que se reconocen en CUALQUIER columna:

```javascript
const FN_WHITELIST = [
    'AB', 'EM', 'CD', 'EF',  // Common naval BOM codes
    // ... más códigos según catalog GESA
];
...
**Rule**: Válidos part numbers tienen patterns específicos:
...
// Examples:
isLikelyPartNumber("ABC123")     → true
isLikelyPartNumber("123456")     → true
isLikelyPartNumber("A1B2C3D4")   → true
isLikelyPartNumber("ABC")        → false  (length < 6)
isLikelyPartNumber("1234")       → false  (no letters, length < 8)
isLikelyPartNumber("ABCD")       → false  (no digits, length < 8)
```

## Measurement Normalization

**Problem**: Medidas pueden estar en múltiples formatos:
- `A 55 X 5` (espacio separado)
- `A  55   X  5` (múltiples espacios)
- `55x5` (sin espacios)
- `55 x 5` (minúsculas)

**Solution**: Normalizar a formato canónico (single spaces):
...
function normalizeMeasurement(text) {
    return String(text || '')
        ...
        .replace(/\s*x\s*/gi, ' X ')  // "x" → " X " (normalized)
        .trim();
}

// Examples:
normalizeMeasurement("A  55   X  5")   → "A 55 X 5"
normalizeMeasurement("A55x5")          → "A 55 X 5"
normalizeMeasurement("55 x 5")         → "55 X 5"
```

## Nullable Fields

Algunos campos pueden estar legitimamente vacíos en ciertas filas:
...
**Whitelist de válidos unit tokens**:
...
**Aplicación**: Si token en `units` column no es válido, marcar como warning.

## Multiline Field Reconstruction

Si un campo tiene múltiples rects (ej: DESIGNATION en 2 líneas), reconstruir texto:

```javascript
function reconstructMultilineField(rects) {
    if (!Array.isArray(rects)) return String(rects?.text || '');
    
    // Sort by X (left-to-right, dentro de la misma línea)
...
**Current status**: Detectado pero no classificado automáticamente (requires manual validation).
...
1. buildTableGrid()  → rows[] con grid sin validación semántica
2. rows.forEach(row => {
...
3. return rows[]  → con semantic validation aplicada
```
...
Grid after:
  row.grid = {
      ...
      qty: Rect("5"),
      units: Rect("PC"),
      weight: undefined,   ← now empty
      fn: Rect("AB"),      ← moved here
      measurement: Rect("A 55 X 5"),
      standard: Rect("ISO 1234")
  }
...
normalizeMeasurement():
  ...
  row.grid.measurement.text = "A 55 X 5"  ← normalized
