...
Detectar automáticamente dónde está el **límite vertical de cada columna** usando:
...
4. **Iterative refinement**: 2 passes de optimización con validaciones
...
function refinePosPartBoundary(columns, bodyRects, separators, ...) {
    // 1. Buscar tokens que parecen "POS" (short integers: 1-3 dígitos)
    // 2. Buscar tokens que parecen "PART NO" (alphanumeric, length >= 6)
    // 3. Calcular median posición final de POSs
    // 4. Calcular median posición inicio de PARTNOs
...
**Manifestation**:
- FN column está casi vacío
- WEIGHT column es anormalmente ancho
- FN tokens (2-3 dígitos) están clasificados como WEIGHT

**Root cause**:
- Ambos son numéricos
- FN es muy estrecho (14-30 px) vs WEIGHT (35-70 px)
- Boundary snapping puede mover boundary a posición subóptima
...
**Ratios explanation**:
- PART NO: 9% → 22% = ~13% ancho (standard GESA)
- DESIGNATION: 22% → 41% = ~19% ancho (longest field typically)
...
Detectar **dónde realmente terminan/comienzan columnas** en body rects.
...
// Gaps que aparecen en ≥22% de rows son "persistent"
const minHits = Math.max(4, Math.round(rows.length * 0.22));
```

**Intuition**: Si el 22% de rows tienen un gap en posición X, ese es probablemente una columna boundary.
...
**Rationale**:
- Pass 1: Ajusta boundaries según evidencia del body
- Pass 2: Limpia edge cases, minimiza overlaps, aplica constraints finales

## Boundary Crossing Metrics

Detecta cuando tokens violan boundaries (signo de mala alineación):
...
Row 2:  2    DEF-456    Gasket          NA    2    PC    2.1kg     --    ø25         DIN 5678
...
Template columns + separators:
  POS:     template x1=50  → snap to gridline 50  → final x1=50
  PART NO: template x1=90  → snap to gridline 95  → final x1=95
  DESIGNATION: template x1=150 → snap to gridline 150 → final x1=150
  ...
...
- qtyStarts (22nd percentile) = posición donde 5/20 comienzan
- if qtyStarts > modelEnds: move boundary to mitad
- Aplicar guard: asegurar QTY ≥ 24px
...
Si MODEL/TYPE o QTY/UNITS/FN/WEIGHT están missing en detección:
...
Cada columna tiene color para visualización:
...
Overlays mostrarán rectángulos semitransparentes en estos colores para cada token asignado a columna.
...
1. buildTemplateColumns() → initial columns[]
2. mergeAnchorsIntoTemplate() → anchor-aware columns
3. detectVerticalSeparators() → separators[] from 3 strategies
4. snapBoundariesToSeparators() → first snap
5. refineVerticalBoundaries() → 2-pass iterative refinement
6. buildBoundaryList() → final boundaries[] array
7. assignRectsToColumns() → asigna body rects a columnas
```
