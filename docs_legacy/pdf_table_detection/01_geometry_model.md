...
El modelo geométrico define cómo se extraen elementos del PDF, se normalizan coordenadas, se aplican transformaciones de escala y se calculan relaciones espaciales. La precisión de este modelo es crítica para todas las etapas downstream (header detection, column detection, row grouping).
...
Después de `extractTextRects()`, cada token se representa como:
...
```javascript
{
  ...
  centerY: 449,               // top - (height / 2) → normalized to top-down
  visualTop: 442              // top - height → for line grouping
}
```

### Coordinate System

**MILU usa un sistema top-down normalizado** (como la mayoría de navegadores web):

- **Y=0** en la parte superior de la página
- **Y crece hacia abajo**
- Conversión desde PDF (bottom-up):
  ```javascript
...
Cada página PDF tiene un viewport que define escala y transformación:
...
**Rationale**: A zoom 100%, tolerancias están calibradas. A zoom 200%, los tolerancias deben crecer proporcionalmente.

## Clustering & Grouping

### Line Grouping (Vertical Baseline Clustering)

Agrupa rects que están horizontalmente alineados (mismo baseline Y):

```javascript
function groupIntoLines(rects, tolerance = 6) {
    // 1. Sort by Y coordinate (centerY)
    // 2. Dinámico tolerance = max(6px, height * 0.55)
    //    → Tokens más grandes permiten desalineación mayor
    // 3. Asign cada rect a line más cercana (≤ dynamicTol)
    // 4. Calcula cy promedio de la línea
    // 5. Ordena rects dentro de línea por X (left edge)
...
Output lines (tolerance=6):
  Line1 (cy=102): [Rect1, Rect2, Rect3] → "POS PART NO."
  Line2 (cy=201): [Rect4, Rect5] → "1 ABC123"
```

### Cluster Building (Horizontal Gap Clustering)

Agrupa rects que están horizontalmente cercanos (gap < threshold):

```javascript
function buildClusters(rects, maxGap = 28) {
    // 1. Sort by X (left)
    // 2. Agrupa rects si gap ≤ maxGap
...
**Uso**: Encontrar posición "típica" robustamente contra outliers.
...
function percentile(values, p = 0.5) {
    ...
    // percentile(values, 0.50) → Median
    // percentile(values, 0.75) → Q3
}
```

**Uso**: 
- `percentile(..., 0.78)` para left-edge detection (78% de rects terminan aquí)
- `percentile(..., 0.22)` para right-edge detection (22% de rects comienzan aquí)
...
function detectHistogramGaps(bodyRects, leftX, rightX, geomScale = 1) {
    ...
    // 4. Find valleys: current ≤ threshold && left > threshold && right > threshold
...
Tolerancias típicas en MILU (en viewport pixels):
...
| Header window | 90 px | geomScale | Búsqueda de headers desde table top |
| Snap window | 8-20 px | geomScale | Boundary snapping a separadores |
| Natural gap min | 5 px | geomScale | Mínimo gap para considerar brecha |
| Boundary crossing | 0.6 px | geomScale | Umbral para detectar token que cruza límite |
...
// Medians/percentiles: mantener precisión
...
// → Line1: {cy: 101, rects: [POS, PART, NO], text: "POS PART NO"}
// → Line2: {cy: 200, rects: [1], text: "1"}
```

---

**Véase también:**
- [02_table_region_detection.md](02_table_region_detection.md) - Uso de geometry para detectar área tabular
