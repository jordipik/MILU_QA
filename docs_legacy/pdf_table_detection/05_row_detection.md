...
**Input**: `bodyRects[]` (tokens dentro del área tabular), `columns[]` (boundaries).
...
**Rationale**: 
- Tokens grandes (height=20) se permiten desalineación mayor
- Tokens pequeños (height=8) requieren alineación cercana
- Mínimo 12 px siempre
...
Row with 2 baselines:
  ...
  distance = |100-115| = 15 px > 12 → separate rows
...
Rects:
  Rect1: left=55, width=20, centerX=65  → falls in POS [50-90] → grid.pos = Rect1
  Rect2: left=100, width=40, centerX=120 → falls in PART NO [90-150] → grid.part_no = Rect2
  Rect3: left=155, width=100, centerX=205 → falls in DESIGNATION [150-350] → grid.designation = Rect3
```

### Multiline Field Handling

Si un campo tiene múltiples líneas (ej: DESIGNATION largo):
...
Post-processing (manual merge or heuristic):
  Si Row2 no tiene POS/PART NO, probablemente es continuación de Row1
  → Merge rects de Row2 a Row1, update grid
```
...
- Referencia única de fila en debug/export
- Tracking en QA system
- Sort order en output

## Metadata per Row

Cada row también puede tener:
...
Asignación depende de `columns[]` boundaries. Si columnas están mal alineadas, la asignación falla:

```
Bad column alignment:
  POS:     [50-120]    ← too wide, incluye PART NO
  PART NO: [120-200]   ← too narrow

Resultado:
  grid.pos = [rect1, rect2]  ← ambos tokens asignados a POS
  grid.part_no = []          ← vacío
```

→ Validación downstream detecta grid.part_no vacío → warning PART_NO_MISSING.
...
Row: "1", [gap], "ABC-123", [gap], "Engine", ...
      ↑ POS              ↑ PART_NO            ↑ DESIGNATION

Si gap es grande, centerX de ABC-123 puede caer en otro rango.
→ grid.pos = [Rect1]
→ grid.part_no = []      ← missing
→ grid.unknown = [Rect2] ← asignado a unknown si X está fuera
```
...
Step 2: Cluster
  Row1 (baselineY=100):
    currentRow = [Rect1]
    Rect2: yDist = 0 <= 12 → add to Row1
    Rect3: yDist = 0 <= 12 → add to Row1
    Rect5: yDist = 0 <= 12 → add to Row1
    Rect6: yDist = 0 <= 12 → add to Row1
    Rect4: yDist = 15 > 12 → new row
...
Step 3: Assign to columns
  ...
  Rect1 centerX=65: in [50-90] → grid.pos = Rect1
  Rect2 centerX=105: in [90-150] → grid.part_no = Rect2
  Rect3 centerX=200: in [150-350] → grid.designation = Rect3
  Rect5 centerX=520: in [500-550] → grid.weight = Rect5
  Rect6 centerX=600: in [550-700] → grid.standard = Rect6
...
- [06_semantic_classification.md](06_semantic_classification.md) - Validación post-assignment
