# DOCUMENTO CANÓNICO MILU — FLUJO DE DATOS

> **Estado**: CANÓNICO · Fuente única de verdad para los flujos de datos.
...
6. [js/revision.js](../js/revision.js) `assignRevisionKeys()` añade claves estables y legacy de revisión por fila.
...
> Variante: carga incremental AR-1 (`?lazy=1`) — solo se cargan los motores seleccionados desde `GET /engines`. Ver [12_ar1_carga_incremental.md](12_ar1_carga_incremental.md).

---

## 2. Flujo interactivo de QA

1. El usuario actualiza filtros / búsqueda / orden.
2. [js/qa-table.js](../js/qa-table.js) `applyFilters()` filtra por flags, texto, valores de revisión y rangos book/page.
3. `sortData()` aplica orden personalizado (`book_page_pos` como referencia principal).
4. `renderTable()` pinta filas visibles según el `column-view` activo.
5. La selección sincroniza el panel derecho (PDF / esquemas / detalle).

---

## 3. Guardado de revisión por fila
...
Salida: `engine_*.json` actualizado con la revisión de esa fila.

---

## 4. Decisión global por PN (PN Review)

1. Usuario abre `pn_review.html` o la pestaña **PN Review** de `analista_02.html`.
...
4. Usuario pulsa **Validar** / **Revisar** / **Descartar** → confirm con `<dialog>` nativo.
...
8. En `analista_02.html`, callback `onDecisionApplied` → `revalidateCurrentRow()` + `renderReviewStats()`.

Resultado: **todos los registros del PN en todos los motores** quedan con el mismo estado de decisión.

> ⚠️ Inconsistencia conocida: `analista_02.js` usa `descartar` mientras la UI principal usa `eliminar`. Tarea **DT-5** del plan ([PLAN_TRABAJO_MILU.md](PLAN_TRABAJO_MILU.md)). **PENDIENTE DE VALIDAR**.

---

## 5. Exportación sintética (offline)
...
## 6. Normalización de campos finales (offline)

`python depuracion_json.py` itera los 9 engine JSON y, por registro:

- Normaliza espacios en `dimensions_gesa` y `MEASUREMENT / STANDARD` (colapsa múltiples espacios a uno).
...
- Corrige el typo `wheight_final` → `weight_final`.
- Recalcula `exp_imagenes` con prioridad `ruta_foto → ruta_esquemas_pos → placeholder`.
...
## 7. Exportación WordPress (oficial)

Ver [WORDPRESS_EXPORT_MILU.md](WORDPRESS_EXPORT_MILU.md). Resumen:

1. `npm run export:wordpress` → [scripts/export_wordpress_milu.js](../scripts/export_wordpress_milu.js).
2. Lee los 9 `engine_*.json`.
3. Agrupa por PN (SKU global).
4. Decisión QA-only:
   - `decision = import` si ∃ fila con `qa_revision_estado=ok` y `qa_revision_accion=importar`.
...
5. Clasificación New / Superseded **solo** por `sust_hierarchie === "Superseded"`.
...
## 8. Diagnóstico de persistencia (orden recomendado)

Cuando algo no se guarda, validar en este orden (regla operativa del proyecto):

1. `GET /health` — ¿está vivo el servidor?
2. `GET/POST /qa_revision_sync.php` — ¿responde JSON desde Express?
3. `POST /save-json` o `POST /apply-revision-to-engines` según el flujo afectado.
...
- **Designación**: `DESIGNATION`, `designation_final`, `designation_gesa`.
- **Métricas**: `MEASUREMENT / STANDARD`, `dimensions_gesa`, **`measure_final`**, `WEIGHT`, `weight_final`, `norma`.
- **Sustitución**: `sust_status`, `sust_hierarchie` (`"New"` | `"Superseded"`).
- **Multimedia**: `ruta_foto`, `ruta_esquemas_pos`, `exp_imagenes` (campo derivado final).
- **QA**: `qa_revision_estado` ∈ `{pendiente, ok}`, `qa_revision_accion` ∈ `{importar, copia, revisar, eliminar}`, `qa_revision_updated_at`.
...
- Canónico: [02_data_flow.md](02_data_flow.md), [03_data_models.md](03_data_models.md)
